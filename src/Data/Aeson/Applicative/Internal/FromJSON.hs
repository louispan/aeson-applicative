...
-- | Function variant of '.:'.
aparseField :: (AFromJSON m a) => Object -> Text -> Parser (m a)
aparseField = explicitParseField aparseJSON
{-# INLINE aparseField #-}
...
-- based on (.:?)
aparseFieldMaybe :: (AFromJSON m a) => Object -> Text -> Parser (Maybe (m a))
aparseFieldMaybe = explicitParseFieldMaybe aparseJSON
...
instance {-# INCOHERENT #-} (Selector s, AFromJSON m a) =>
  AFromRecord m arity (S1 s (K1 i (Maybe a))) where
    aparseRecord opts _ obj = fmap (M1 . K1) <$> obj `aparseFieldMaybe_` pack label
        where
          aparseFieldMaybe_ :: (AFromJSON m a) => Object -> Text -> Parser (m (Maybe a))
          aparseFieldMaybe_ o t = sequenceA <$> (aparseFieldMaybe o t)
